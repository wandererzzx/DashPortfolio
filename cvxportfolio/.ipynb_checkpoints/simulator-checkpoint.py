...
class MarketSimulator():
    ...
    def __init__(self, yc_market_returns, costs,
                 market_volumes=None, cash_key='cash'):
        """Provide market returns object and cost objects."""
        self.yc_market_returns = yc_market_returns
    ...
    def propagate(self, h, u, t):
        ...
        # close out halted symbols for free
        is_halted = (self.yc_market_returns[t:].std(axis=0).drop(self.cash_key) == 0.0)
        is_halted.loc[self.cash_key] = False
        u[is_halted] = 0.0
        ...
        is_halted.loc[self.cash_key] = False
        hplus[is_halted] = 0.0
        u[is_halted]     = hplus[is_halted] - h[is_halted]
        assert not np.any(h + u - hplus)

        assert (hplus.index.sort_values().equals(self.yc_market_returns.columns.sort_values()))

        # apply future returns
        h_next = self.yc_market_returns.shift(-1).loc[t] * hplus + hplus

        assert not np.any(h_next[is_halted])
    ...
    def run_backtest(self, initial_portfolio, start_time, end_time,
        ...
        market_returns_index = self.yc_market_returns.index[:-1]
        simulation_times = market_returns_index[
            (market_returns_index >= start_time) &
            (market_returns_index <= end_time)]
        ...
        for t in simulation_times:
            ...
            results.log_simulation(t=t, u=u, h_next=h,
                                   risk_free_return=self.yc_market_returns.shift(-1).loc[
                                       t, self.cash_key],
                                   exec_time=end - start)
